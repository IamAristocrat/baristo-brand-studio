...
export function ReservationLayer() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await response.json()) as { ok?: boolean; reservationId?: string; message?: string };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-obsidian/75 px-4 py-6 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="reservation-title" className="max-h-full w-full max-w-2xl overflow-y-auto rounded-2xl border border-rosegold/30 bg-ivory shadow-luxe">
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-rosegold/15 bg-ivory/95 px-5 py-4 backdrop-blur-xl sm:px-7">
          <div>
            ...
            <h2 id="reservation-title" className="mt-1 font-display text-3xl font-semibold text-espresso">Reserve {roast}</h2>
          </div>
          <button type="button" onClick={close} disabled={status === "submitting"} aria-label="Close reservation" className="flex h-10 w-10 items-center justify-center rounded-full border border-rosegold/20 text-espresso hover:bg-champagne/35 disabled:opacity-40"><X className="h-4 w-4" /></button>
        </div>
        ...
        {status === "success" ? (
          <div className="px-6 py-12 text-center sm:px-10">
            ...
            <p className="mt-4 text-sm leading-7 text-espresso/65">Reference <strong>{reservationId}</strong>. Acknowledgement has been sent to {form.email}. Baristo will verify availability, delivery eligibility and the final payable amount before sending a secure payment link.</p>
            <div className="mx-auto mt-7 max-w-md rounded-xl border border-rosegold/18 bg-white/65 p-5 text-left text-sm leading-7 text-espresso/70">
              ...
              <p className="mt-3 text-xs text-espresso/50">This reservation is not a confirmed order. Dispatch begins only after availability verification and successful payment.</p>
            </div>
            <button type="button" onClick={close} className="smallcaps mt-8 rounded-sm bg-gradient-rose px-7 py-3 text-xs font-bold text-espresso shadow-rose">Return to Baristo</button>
          </div>
        ) : (
          <form onSubmit={submit} className="space-y-6 px-5 py-6 sm:px-7">
            <div className="rounded-xl border border-rosegold/18 bg-white/65 p-4 text-sm text-espresso/70">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div><strong className="font-display text-xl text-espresso">{roast}</strong><p className="text-xs">{PACK} · {formatInr(UNIT_PRICE)} per pack</p></div>
                <div className="flex items-center rounded-full border border-rosegold/25 bg-ivory">
                  <button type="button" aria-label="Decrease quantity" onClick={() => setForm((value) => ({ ...value, quantity: Math.max(1, value.quantity - 1) }))} className="flex h-9 w-9 items-center justify-center"><Minus className="h-3.5 w-3.5" /></button>
                  <span className="min-w-8 text-center font-semibold">{form.quantity}</span>
                  <button type="button" aria-label="Increase quantity" onClick={() => setForm((value) => ({ ...value, quantity: Math.min(10, value.quantity + 1) }))} className="flex h-9 w-9 items-center justify-center"><Plus className="h-3.5 w-3.5" /></button>
                </div>
              </div>
              <p className="mt-3 border-t border-rosegold/12 pt-3"><strong>Provisional total: {formatInr(total)}</strong> <span className="text-xs opacity-60">before any confirmed delivery charge</span></p>
            </div>
            ...
            <label className="sr-only" aria-hidden="true">Company<input tabIndex={-1} autoComplete="off" value={form.company} onChange={(event) => setForm((state) => ({ ...state, company: event.target.value }))} /></label>
            ...
            {status === "error" && <div role="alert" className="rounded-xl border border-red-300 bg-red-50 p-4 text-sm text-red-800">{message}</div>}
            ...
            <p className="text-center text-[11px] leading-5 text-espresso/45">No payment is collected on this page. Baristo will send a secure payment link only after verification.</p>
          </form>
        )}
      </section>
    </div>
...
function Field({ label, value, onChange, type = "text", required, ...inputProps }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean } & Omit<React.InputHTMLAttributes<HTMLInputElement>, "value" | "onChange" | "type" | "required">) {
  return (
    <label className="block text-xs font-semibold text-espresso/75">
      {label}{required && <span aria-hidden="true"> *</span>}
      <input {...inputProps} required={required} type={type} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 h-12 w-full rounded-md border border-rosegold/20 bg-white px-3 text-sm text-espresso outline-none transition focus:border-rosegold focus:ring-2 focus:ring-rosegold/15" />
    </label>
  );
}
